...
#' @param cumulative Logical, if TRUE the cumulated impulse response coefficients are computed. The default value is false.
...
#'
#'
#'@export wild.boot.mod



wild.boot.mod <- function(x, design = "fixed", distr = "rademacher", n.ahead = 20, cumulative = FALSE,
  ...
  bootf <- function(Ustar1){
    ...
    ip <- irf(temp, n.ahead = n.ahead, cumulative = cumulative)
  ...
  ip <- irf(x, n.ahead = n.ahead, cumulative = cumulative)
